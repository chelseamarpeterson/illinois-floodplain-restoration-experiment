...
soil.df = read.csv("floodplain-experiment-repo/Soil_Analysis/Clean_Data/Soil_Data_by_Quadrat_June2023.csv")
...
leave.out = c("texture.class","n.release","volumetric.moisture","coarse.material",
              "tc.stock","toc.stock","maoc.stock","poc.stock","tic.stock", ag.cols, meq.cols)
soil.df = soil.df[,-which(colnames(soil.df) %in% leave.out)]

# read in biomass data
bm.df = read.csv("floodplain-experiment-repo/Understory_Analysis/Clean_Data/Understory_Material_Mass_By_Quadrat_Sep2022.csv")

# combine biomass and soil data
soil.bm.df = left_join(soil.df, bm.df, by=c("full.treatment.name","treatment","plot","quadrat"))

# remove reference
soil.bm.df = soil.bm.df[-which(soil.bm.df$treatment == "R"),]

# update column names
colnames(soil.bm.df)[6:38] = c("Temp","Moist","BD","Root.Frag",text.cols,"pH",
                               "P","K","Ca","Mg","SOM","NO3","NH4","CEC","TN",
                               "TC","SOC","POC","EV","HT","MWD","TIC","MAOC",
                               "POC_MAOC","CN","HB","HL","FWD","MB","PAB","HJB")

# select explanatory and response variables
y.cols = c("SOC","POC","MAOC","POC_MAOC")
x.cols = c("Temp","Moist","BD","Root.Frag","TN","CN","Sand","Clay","pH","P","NO3",
           "NH4","CEC","EV","HT","MWD","HL","FWD","MB","PAB","HJB")
...
# write plot
ggsave("Manuscript/Main_Figures/Figure5_Soil_Vegetation_RDA_HTElev.jpeg", 
