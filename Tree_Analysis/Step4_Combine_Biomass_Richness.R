...
c.data = c.data %>%
         mutate(total.abg.wood.carbon = abg.live.stem.carbon + abg.live.tree.carbon,
                ...
                total.fwd.carbon = intermediate.fwd.carbon + small.fwd.carbon,
                total.snag.carbon = large.snag.carbon + dead.stem.carbon,
...
colnames(total.sp.df) = c("treatment","plot","n.herb.only","n.tree.only","n.both.herb.tree","n.total")
k = 1
for (t in 1:n.t) {
  for (n in 1:n.p) {
